...
package io.github.xiaotong6666.fusehide.gradle
...
abstract class AdbModuleTask : DefaultTask() {
    ...
    private fun rootOutput(
        serial: String,
        access: RootAccess,
        command: String,
    ): String? = adbOutput(serial, "shell", rootCommand(access, command))
    ...
    private fun commandAvailable(
        ...
        access: RootAccess,
        test: String,
    ): Boolean = rootOutput(serial, access, "$test && echo available")
        ?.lineSequence()
    ...
    fun run() {
        ...
        targetDevices().forEach { serial ->
            ...
            try {
                ...
                logger.error("Failed on device $serial: ${exception.message}")
