...
constexpr char kFrameworkRelPath[] = "framework";
...
bool PreloadModuleRuntime(zygisk::Api* api, JNIEnv* env) {
    ...
    const int frameworkDirFd =
        openat(moduleDirFd, kFrameworkRelPath, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
    char libraryPath[96];
    ...
    const bool dexLoaded = PreloadInjectedDex(env, frameworkDirFd);
    close(frameworkDirFd);
    if (!dexLoaded) {
        close(libraryFd);
        return false;
